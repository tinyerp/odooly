...
_term_re = re.compile(
    r"([\w._]+)\s*"
    r"(=(?:like|ilike|\?)|[<>]=?|!?=(?!=)"
    r"|(?<= )(?:like|ilike|in|not like|not ilike|not in|child_of))"
    r"\s*(.*)"
)
...
if PY2:
    ...
    class _DictWriter(csv.DictWriter):
        ...
        def _dict_to_list(self, rowdict):
            ...
            return [
                cell.encode("utf-8") if hasattr(cell, "encode") else cell
                for cell in rowlst
            ]
...
def format_exception(
    ...
    elif issubclass(exc_type, Fault) and isinstance(  # XML-RPC
        exc.faultCode, basestring
    ):
    ...
    if server_error:
        ...
        if server_error["exception_type"] != "internal_error" or message.startswith(
            "FATAL:"
        ):
            server_tb = None
        else:
...
class Env(object):
    ...
    def __repr__(self):
        return "<Env '%s@%s'>" % (self.user.login if self.uid else "", self.db_name)
    ...
    def check_uid(self, uid, password):
        ...
        """
        try:
            self.client._object.execute_kw(
                self.db_name, uid, password, "ir.model", "fields_get", ([None],)
            )
        except Exception:
    ...
    def _upgrade(self, modules, button):
        ...
        if sel:
            ...
            if any(mod["name"] not in modules for mod in mods):
                raise Error(
                    "Pending actions:\n"
                    + "\n".join(("  %(state)s\t%(name)s" % mod) for mod in mods)
                )
...
class Client(object):
    ...
    def create_database(
        ...
        if login == "admin" and not country_code:
            self.db.create_database(passwd, database, demo, lang, user_password)
        elif self.version_info < 9.0:
            raise Error("Custom 'login' and 'country_code' are not supported")
        else:
            self.db.create_database(
                passwd, database, demo, lang, user_password, login, country_code
            )
...
class BaseRecord(BaseModel):
    ...
    def __repr__(self):
        if len(self.ids) > 16:
            ...
            ids = self.id
        return "<%s '%s,%s'>" % (self.__class__.__name__, self._name, ids)
...
def _interact(global_vars, use_pprint=True, usage=USAGE):
    ...
    try:
        ...
        atexit.register(rl.write_history_filewrite_history_file, HIST_FILE)
...
def main(interact=_interact):
    ...
    if args.env:
        ...
        client = Client(
            args.server, args.db, args.user, args.password, verbose=args.verbose
        )
    ...
    if args.model and client.env.uid:
        ...
        writer = _DictWriter(
            sys.stdout, args.fields, "", "ignore", quoting=csv.QUOTE_NONNUMERIC
        )
        writer.writeheader()
