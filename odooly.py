...
class WebAPI:
    ...
    def __call__(self, model='index'):
        return self._request(f'{model}.json') if self._endpoint == '/doc' else None
    ...
    def _request(self, path, params=None):
        ...
        with self._printer as log:
            log.print_sent(f"{verb} {self._endpoint}/{path} {snt}".rstrip())
            res = self._dispatch(path, params)
            log.print_recv(repr(res))
        return res

    def _get_doc_api(self, client):
        if client.version_info >= 19:
            api_doc = self.__class__(client, 'doc', [])
            return api_doc.dummy() and api_doc
...
class Json2:
    ...
    def doc(self, model='index'):
        """Documentation of the `model`."""
        return self._request(f'{self._doc_endpoint}/{model}.json')

    def _get_doc_api(self, client):
        return self.doc('res.device') and self.doc
...
class Env:
    ...
    def set_api_key(self, api_key, store=True):
        ...
        if self.client.web and self.client.version_info >= 19.0:
            ...
            try:
                self._doc = (self._json2 or self.client.web)._get_doc_api(self.client)
            except ServerError:
                self._doc = None
...
class Client:
    ...
    def _proxy_web(self, name):
        if name == 'doc':
            def dispatch_web(leaf, params):
                return self._http.request(urljoin(self._server, f"doc/{leaf}"), method='GET')
...
class Model(BaseModel):
    ...
    def _methods(self, name=''):
        ...
        """
        try:
            methods = self._doc['methods']
        except Exception:
            return {key: args for (key, args) in _base_method_params if name in key}
        dict_methods = {}
        for key, vals in methods.items():
            if name in key:
                arg_names = list(vals['parameters'])
    ...
    def __getattr__(self, attr):
        if attr == '_fields':
            ...
            if vals is None:
                vals = self._doc['fields'] if self._doc else self._execute('fields_get')
